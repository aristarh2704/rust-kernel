...
// Represents continuous piece of some data, for example, VMDescriptor is a continuous piece of virtual memory
...
impl<T> Continuous<T>{
    ...
    fn expand(&mut self,next:Continuous<T>)->Result<(),Continuous<T>>{
        if self.data+self.length==next.length{
